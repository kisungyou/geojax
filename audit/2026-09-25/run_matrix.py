...
def main():
    ...
    parser.add_argument("--environments", nargs="+", help="selected pinned environments")
    ...
    supported = config["tox"]["env_list"].split()
    names = arguments.environments or supported
    if len(names) != len(set(names)) or set(names) - set(supported):
        parser.error("environments must be unique entries from tox.ini")
    # Exercise the oldest dependency floor and newest Python first.
    priority = ["py311-min-float32", "py314-stable-float64"]
    names = [name for name in priority if name in names] + [
