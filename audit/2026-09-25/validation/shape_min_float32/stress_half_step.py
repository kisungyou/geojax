"""Analytic short-geodesic stress check, without modifying package defaults."""
from pathlib import Path
import dataclasses,gc,json,platform,sys,time,traceback
import jax
...
output=Path(sys.argv[1]);start=time.monotonic();rows=[]
...
def save():
 d={'python':platform.python_version(),'jax':jax.__version__,'numpy':np.__version__,'precision':'float32','initial_stepsize':.5,'normalize_step':False,'point_tolerance':4e-5,'cases':rows,'elapsed_seconds':time.monotonic()-start,'completed':len(rows),'passed':sum(r.get('passed',False) for r in rows),'failed':sum(not r.get('passed',False) for r in rows)}
...
for m,rank in [(2,2),(3,2),(4,3)]:
 ...
 for n,separation,uniform in specs:
  ...
  try:
   ...
   fitted=frechet_mean(M,values,sample_weight=raw_weights,solver=solver);assert fitted.diagnostics['effective_tolerance']==tol
   npweights=np.asarray(weights,dtype=np.float64);npweights/=npweights.sum();mean_angle=float(np.dot(npweights,np.asarray(angles,dtype=np.float64)))
