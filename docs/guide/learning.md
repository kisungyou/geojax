...
dense result. The returned matrix still requires
$O(n_\mathrm{left}n_\mathrm{right})$ memory; blocking controls intermediate
geometry evaluations, not the output size.
...
of that iteration rather than global optimality on an arbitrary manifold.
Clustering provides intrinsic Lloyd updates, medoids, valid metric-space
linkage rules, spectral graph methods, mean shift, and competitive quantization
...
distance is Euclidean. Classical scaling normalizes the distance scale before
forming its Gram matrix and reports both normalized and physical spectra. It
raises when squaring the physical scale is not representable in the selected
dtype instead of returning overflowed diagnostics. GeoJAX's `phate` follows
the PHATE diffusion-potential construction but uses classical scaling for the
final metric embedding; it is a dependency-free dense approximation rather
than the reference package's metric-MDS optimizer.

Dense spectral and graph routines materialize $n\times n$ distance, affinity,
or Laplacian matrices. They therefore require $O(n^2)$ memory, and dense
eigendecompositions can require $O(n^3)$ work. For large datasets, reduce the
sample set first or use the mini-batch and coreset routines rather than treating
`block_size` as a sparse-graph substitute.
...
Permutation calibration is finite-sample valid only under exchangeability of
the pooled labels. A distance statistic need not characterize equality of
distributions on every metric space; any stronger consistency claim requires
the assumptions in the cited result.
...
`NearestCentroidModel.predict_proba` converts negative squared distances into a
Gibbs score with a query-local scale. Those rows sum to one, but they are not
calibrated posterior probabilities. Centroid and tangent-feature models expose
whether their internal Fréchet reference fits converged; callers should inspect
`converged`, `reason`, and the stored fit diagnostics before treating a model as
fully resolved.
...
full-batch estimators. A convergence flag additionally requires a full-data
stationarity check, so learning-rate decay alone cannot masquerade as
convergence. Competitive quantization applies the same distinction between a
flat objective history and stationary prototypes.
...
For partially labeled data, `label_propagation` diffuses class scores with a
symmetric normalized affinity operator and hard-clamps known labels after each
update
